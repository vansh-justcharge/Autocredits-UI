...
import { Search, Bell, User, LogOut } from 'lucide-react';
// Import your modal components and Navbar as in your original code:
import AddCar from "./AddCar";
import EditCarModal from "./EditCarModal";
import ViewCarModal from "./ViewCarModal";
import Navbar from "./Navbar";
...
const InventoryPage = () => {
  ...
  const handleAddCar = (carData) => {
    setCars([...cars, { ...carData, _id: Date.now().toString() }]);
    setShowAddCar(false);
  };

  const handleEditCar = (id, carData) => {
  ...
  const handleDeletePopup = () => {
    handleDeleteCar(deletePopup.id);
  };
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      {/* Modals */}
      {showAddCar && (
        <AddCar isOpen={showAddCar} onClose={() => setShowAddCar(false)} onCarAdded={handleAddCar} />
      )}
      {isEditModalOpen && selectedCar && (
        <EditCarModal
          isOpen={isEditModalOpen}
          onClose={() => setIsEditModalOpen(false)}
          car={selectedCar}
          onUpdate={(data) => handleEditCar(selectedCar._id, data)}
        />
      )}
      {isViewModalOpen && selectedCar && (
        <ViewCarModal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} car={selectedCar} />
